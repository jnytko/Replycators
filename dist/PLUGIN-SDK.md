...
Quick Actions appear at the top of the Dashboard, above all plugin widgets. They provide one-click access to the most frequently used plugin workflows.

### Eligibility

| Add a Quick Action when... | Do NOT add one when... |
|---|---|
| Plugin is used every session or multiple times per day | Plugin is used only occasionally |
| One-click access saves meaningful time | Plugin requires prior context or setup before it is useful |
| Plugin is useful immediately after opening the extension | Plugin is a developer template, game, or entertainment feature |
| Plugin provides universal platform utility | A similar Quick Action already exists |

### Registration

Add a `<button class="rc-action-card">` inside `#rc-quick-actions-section > .rc-quick-actions__grid` in `dashboard.html`, **before** the platform "Options" card:
...
`data-plugin-action` is mandatory - `applyPluginVisibility()` uses it to automatically hide the card when the plugin is disabled.

### Label conventions

- 2-3 words, verb phrase: "Extract SF Case", "Get OrgID", "Search Tabs"
- Match the primary action, not the plugin name
- No trailing punctuation

### Ordering

Order by frequency of use. The platform "Options" card must always be last.
